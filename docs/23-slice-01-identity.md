...
### البنية
- [ ] الشجرة في بند ٢ مطابقة — مفيش `src/Contexts/Tenancy/` (ADR-011)
- [ ] `Tenant` في `Src\Support\Domain\Models\` (ADR-011)
- [ ] الأربع قواعد المعمارية في بند ٣ بتعدّي كاختبارات
- [ ] `composer lint` أخضر (Pint + PHPStan level 6)

### التفويض
- [ ] `UserPolicy` بكل دوالها، كلها بترجّع `Response` (`docs/19`)
- [ ] الدوال اللي بتاخد سجل بتستخدم `decideFor()` (ADR-007)
- [ ] `Gate::before` بيحترم `invariants()` **و** `TenantBoundary` (ADR-005)
- [ ] كل صلاحية في الكتالوج لها Gate · `access.panel.admin` شغّالة (ADR-003)
- [ ] صفر `hasPermissionTo` بره `Decision` · صفر `can('x.y')` · صفر `skipAuthorization`

### العزل
- [ ] `UserResource::getEloquentQuery()` بيفلتر بالعضوية (بند ٤)
- [ ] كل الأدوار — بما فيهم `super_admin` — بياخدوا 404 على سجل مستأجر تاني (ADR-005)
- [ ] استعلام بدون سياق بيرمي مش بيرجّع صفوف
- [ ] الأدوار معزولة بين المستأجرين (`teams => true`)

### الاختبارات
- [ ] كل اختبارات بند ٦ خضراء **على PostgreSQL**
- [ ] `TenantContext` بيترجّع في `beforeEach` (بند ٧-٢)
- [ ] الاختبارات بتعدّي بـ `--parallel` وبترتيب عشوائي
- [ ] صفحة الجدول ≤ ١٠ استعلامات

### الواجهة والترجمة
- [ ] `admin` بيفتح اللوحة · `viewer` بياخد 403 على `create`
- [ ] صفر نص مكتوب · صفر `left`/`right` · صفر لون مباشر
- [ ] لقطات: عربي فاتح · عربي داكن · إنجليزي

### البوابة النهائية
- [ ] **مراجعة عقود:** كل عقد في `docs/19` و`docs/20` اتعلّم عليه **اتثبت** أو **اتكسر**
- [ ] أي عقد اتكسر → ADR جديد في `docs/21` **قبل** ما نكمّل
