...
**الأرقام:** ٧٢ اختبار · ١٩٦ تأكيد · **صفر `skip`** · على PostgreSQL 18.6 حقيقي ·
...
| `decideFor()` لكل دالة بتاخد سجل (ADR-007) | ✅ اتثبت | اختبار معماري بيقرا توقيع كل دالة |
...
| الشكل المعياري للحقل الحسّاس (ADR-009) | ⚠️ **مااتجرّبش** | الشريحة مافيهاش حقل مربوط بصلاحية. الآلية اتحقّق منها من كود Filament في [ADR-014](21-decisions.md#adr-014)، بس مافيش اختبار سلوكي. |
...
١. **الشكل المعياري للحقل الحسّاس** — محتاج أول حقل مربوط بصلاحية عشان يتجرّب سلوكياً.
٢. **`authorizeIndividualRecords()`** — الإجراء الجماعي متكتوب صح ومااتجرّبش.
٣. **اللقطات** — مؤجّلة لمهمة الواجهة.
٤. **ميدلوير `AssignRequestContext` و`SetLocale` و`SecurityHeaders`** — مواصفاتهم في
