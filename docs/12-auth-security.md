...
> **الانتحال هو المسار الرسمي والوحيد للوصول لبيانات مستأجر تاني.** (ADR-005)
>
> المدير العام محبوس في سياق المستأجر الحالي زيّه زيّ أي حد — مفيش تجاوز شامل في
> `Gate::before`، ومفيش وصول بالـ ID المباشر لسجل مؤسسة تانية. لما الدعم الفني يحتاج
> يشوف بيانات عميل، بينتحل شخصية مستخدم عنده، فالوصول بيبقى **مسجّل ومحدود بوقت**.
>
> الشرح الكامل في `docs/19` بند ٥ والقرار في `docs/21-decisions.md` → ADR-005.
>
> ⚠️ عشان كده القواعد الستة تحت **مش اختيارية**. دي مش تحسينات UX — دي الضمانة الوحيدة
> إن الوصول العابر للمستأجرين مرئي ومحاسَب عليه.
...
4. **تسجيل الدخول والخروج** في `activity('security')` — مع `tenant_id` بتاع المستأجر المستهدف،
   عشان يبقى فيه إجابة مكتوبة على «مين شاف بيانات مين وإمتى». ده الأثر الوحيد للوصول العابر
   للمستأجرين، فمفيش انتحال من غير تسجيل.
5. **مدة قصوى** — الجلسة المنتحلة بتنتهي بعد **٣٠ دقيقة** تلقائياً، وبترجّع للمستخدم الأصلي
   وسياق مستأجره. المدة دي مش قابلة للتمديد من الواجهة.
