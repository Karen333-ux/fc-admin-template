...
6. **العزل بيسري على كل الأدوار — بما فيهم `super_admin`.** مفيش استثناء ومفيش تجاوز شامل.
   `Gate::before` بيحترم حدود المستأجر عبر `TenantBoundary` (`docs/19` بند ٥ · ADR-005).
   الوصول لبيانات مستأجر تاني بيحصل **بس** عن طريق انتحال الشخصية (`docs/12` بند ٣).
7. **`withoutScope()` ممنوع** إلا بالشروط الأربعة دي مجتمعة:
   - في **أمر console أو job مجدول بس** — مفيش `withoutScope()` في طلب HTTP إطلاقاً
   - عليه تعليق بيشرح ليه
   - **قراءة بس** — مفيش كتابة عبر المستأجرين
   - النتيجة **مجمّعة (aggregate)** مش صفوف خام لمستأجر معيّن
...
**وبيمرّ على كل الأدوار كمان — من غير استثناء لـ `super_admin`.** الاستثناء في اختبار أمني هو
بالظبط المكان اللي الباگ بيختبي فيه. الاختبارات في `docs/19` بند ١٠.

> **مصيدة الآلية:** `Gate::before` بترجّع `null` عند عبور الحد عشان الـ Policy تشتغل — يعني
> لو الـ Policy **مفيهاش** `ruleOrNotFound` بتاع المستأجر، المدير العام هيعدّي. الحمايتين
> مكمّلتين لبعض. الاختبار المعماري في `docs/19` بند ٩ بيرفض أي Policy ناسية الحارس.
