...
> قاعدة بتمنعه). عشان كده كل دالة في Policy بتاخد موديل تابع لمستأجر **لازم** يبقى عندها
> حارس المستأجر.
>
> **الحارس بقى تلقائي — متكتبوش بإيدك.** [ADR-007](21-decisions.md#adr-007) نقله جوّه
> `decideFor()`، فالشكل الصح:
>
> ```php
> return $this->decideFor($record)        // ← الحارس اتطبّق هنا تلقائياً
>     ->permission($user, $this, 'view')
>     ->response();
> ```
>
> الشكل اليدوي القديم — `->ruleOrNotFound($record->tenant_id === ...)` في أول كل دالة —
> **مابقاش مطلوب**، ومكتوبه دلوقتي بيبقى تكرار للي `decideFor()` عمله. لسه ينفع تستخدم
> `ruleOrNotFound()` لقواعد إخفاء تانية، بس **مش** لحارس المستأجر.
>
> الاختبار المعماري في بند ٩ بيفحص إن كل دالة بتاخد سجل بتستخدم `decideFor(` —
> مش `ruleOrNotFound`.
