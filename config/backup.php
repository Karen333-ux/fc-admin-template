...
return [
    ...
    'backup' => [
        ...
        'destination' => [
            ...
            /*
             * الديسك اللي النسخ الاحتياطية بتتخزن عليه — s3-private
             * الموجود أصلاً (config/filesystems.php، docs/04 بند ٢) مش
             * ديسك جديد. النسخ الاحتياطية حساسة زي المستندات الخاصة
             * بالظبط، فـ visibility=private مناسبة، مفيش داعي لدلو مخصص
             * زي s3-backups في مثال الوثيقة. (docs/11 بند ٩)
             */
            'disks' => [
                's3-private',
            ],
    ...
    'notifications' => [
        ...
        'mail' => [
            // ⚠️ لازم قيمة صالحة دايماً — spatie/laravel-backup بيتحقق من
            //    شكل الإيميل وقت بناء الـ Config (`NotificationMailConfig::fromArray()`)
            //    مش بس وقت إرسال إشعار فعلي، فـ null بيرمي InvalidConfig
            //    حتى لو الإشعارات نفسها مش شغّالة. نفس default القيمة
            //    الافتراضية في config/mail.php (مش config() متبادل بين
            //    ملفات — ترتيب تحميل config الأبجدي بيخلّي mail.php
            //    لسه مش متحمّل وقت ما backup.php بيتحمّل). (docs/11 بند ٩)
            'to' => env('BACKUP_ALERT_EMAIL', 'hello@example.com'),
